...
def radiative_source_term(
    radiative_algae=0.0, radiative_ice=0.0, radiative_organicmatter=0.0
):
...
def radiative_organicmatter(depth):
    ...
    kappa_attenuation_coefficient = (
        1.5 + a_det_organic_matter
    )  # of organic matter in m-1 + ice in m-1

    I_0 = (
        i0_surface_transmission_parameter
        * (1 - albedo_organicmatter)
        * F_sw_solarradiance
    )
    I_z = I_0 * np.exp(-kappa_attenuation_coefficient * depth)
...
def calculate_radiative_terms(
    depth, thickness_index, radiative_algae=0.0, algae_model_depth_type="single"
):
    ...
    return radiative_source_term(
        radiative_algae=radiative_algae_all_depth,
        radiative_ice=radiative_ice_array,
        radiative_organicmatter=radiative_organicmatter,
    )


def calculate_local_rayleigh_number(
    thickness_index,
    thickness,
    salinity,
    phi,
    ...
    h_i = np.linspace(
        0, z, thickness_index
    )  # height at each layer until the interface within the sea ice
    ...
    for i in range(thickness_index):
        if len(h_i) == 0:
            min_pi_i = min(pi_i[i:])
            Ra_i[i] = (
                gravity * rho_sw * beta * (salinity[i] - S_sw) * min_pi_i * (z - 0)
            ) / (kappa * mu)
        else:
            min_pi_i = min(pi_i[i:])
            Ra_i[i] = (
                gravity * rho_sw * beta * (salinity[i] - S_sw) * min_pi_i * (z - h_i[i])
            ) / (kappa * mu)
...
def calculate_salinity_flux(dz, dt, Ra_c, thickness_index, thickness, salinity, phi):
    ...
    Ra_i = calculate_local_rayleigh_number(
        thickness_index, thickness, salinity, phi, grid_size=dz
    )
...
def calculate_brine_flux(
    thickness_index, thickness, interface_depth, salinity, phi, dt, dz, Ra_c=10
):
    ...
    brine_flux = calculate_salinity_flux(
        dz, dt, Ra_c, thickness_index, thickness, salinity, phi
    )
...
def calculate_salinity_source_term_from_brineflux(
    salinity, flux_in, flux_out, flux_channel
):
    ...
    for i in range(len(source_term)):
        if i == 0 and len(source_term) > 1:
            source_term[i] = (
                salinity[i + 1] * flux_in[i + 1] - salinity[i] * flux_channel[i]
            )
        elif len(source_term) > 0:
            source_term[i] = (
                salinity[i + 1] * flux_in[i]
                - salinity[i] * flux_channel[i]
                - salinity[i] * flux_out[i]
            )
        else:
            source_term[i] = 0.0
...
def get_salinity_source_term(thickness_index, thickness, salinity, phi, dt, dz):
    ...
    get_brine_flux = calculate_brine_flux(
        thickness_index, thickness, interface_depth, salinity, phi, dt, dz, Ra_c=10
    )
    get_influx, get_outflux = calculate_in_out_brine_flux(
        thickness_index, get_brine_flux, phi
    )
    get_source_term = calculate_salinity_source_term_from_brineflux(
        salinity, get_influx, get_outflux, get_brine_flux
    )

    if len(get_source_term) < len(salinity):
        get_source_term = np.append(
            get_source_term, np.zeros(len(salinity) - len(get_source_term))
        )
